...
# MongoDB setup
DATABASE_URL = os.environ.get("DATABASE_URL", "mongodb+srv://w@cluster0.krzxuop.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
async def send_test_to_user(client, message, html_content: str, test_name: str, test_time: str, questions_count: int, file_id: str = None):
    ...
    LOG_CHANNEL,
    html_filename,
    caption=f"📤 Sent to {message.from_user.mention if message.from_user else 'Unknown User'}\n\n"
            f"📝 {test_name}\n"
...
• ᴘʀᴇᴍɪᴜᴍ ᴋᴇ ʟɪʏᴇ @Ganesh_chouhan_143 ᴘᴀʀ ᴍᴇꜱꜱᴀɢᴇ ᴋᴀʀᴇ"""
...
async def get_welcome_buttons(user_id: int = None):
    ...
    buttons.append([
        InlineKeyboardButton("📢 ᴜᴘᴅᴀᴛᴇs", url="https://t.me/+2EkhKHVIro5mNWQ1")
    ])
...
💰 **ᴘʱɪᴄᴇ:** ₹500 ᴏɴʟʏ!"""
    ...
    buttons = [
        [InlineKeyboardButton(
            "🛍️ ʙᴜʏ ɴᴏᴡ",
            url=f"https://t.me/Ganesh_chouhan_143"
        )],
        ...
        if mode == 'premium':
            ...
            if not user.get('is_premium', False):
                await callback_query.answer(
                    ...
                    "Contact @Ganesh_chouhan_143 to get premium access.",
                    show_alert=True
                )
                return
        else:  # demo mode
...
async def handle_account_callback(client, callback_query):
    ...
    try:
        ...
        account_msg = (
            ...
            f"📞 ᴄᴏɴᴛᴀᴄᴛ @Ganesh_chouhan_143 ғᴏʀ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss"
        )
...
@app.on_message(filters.command(["addxganesh", "removexganesh"]) & filters.private)
async def handle_premium_command(client, message):
    ...
    try:
        ...
        if len(args) != 2:
            await message.reply(
                "❌ Format: /addxganesh user_id days\n\n"
                "Example:\n/addxganesh 123456789 30"
            )
            return
            
        user_id = int(args[0])
        days = int(args[1])
        is_add = message.command[0].lower() == "addxganesh"
...
async def register_user(message):
    """Register a new user or fetch existing"""
    try:
        ...
        return await db.users.find_one({"user_id": user.id})

    except Exception as e:
        print(f"Error registering user: {e}")
        return None  # Important: return None so handler knows it failed
...
async def handle_referral(client: Client, message: Message, ref_code: str):
    ...
    try:
        # Get referrer info using referral code
        ...
        # Update referrer's credits and mark referral used
        await db.users.update_one(
            {"user_id": referrer_id},
            {
                "$inc": {"credits": 5},  # reward
                "$addToSet": {"referrals": new_user_id}
            }
        )

        # Mark new user as having used referral
        await db.users.update_one(
            {"user_id": new_user_id},
        ...
        try:
            ...
            await client.send_message(
                referrer_id,
                f"🎉 ᴄᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴꜱ!\n\n"
                f"📱 {referee_name} ᴜꜱᴇᴅ ʏᴏᴜʀ ʀᴇꜰᴇʀʀᴀʟ ʟɪɴᴋ.\n"
                f"🎁 5 ᴄʀᴇᴅɪᴛꜱ ᴀᴅᴅᴇᴅ ᴛᴏ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ!"
            )
        except Exception as notify_err:
            logger.error(f"Error notifying referrer: {notify_err}")

    except Exception as main_err:
        logger.error(f"Error handling referral: {main_err}")
        ...
        if not await is_premium(user_id):
            ...
            if credits <= 0:
                await message.reply(
                    ...
                    "📞 ᴄᴏɴᴛᴀᴄᴛ @Ganesh_chouhan_143 ᴛᴏ ɢᴇᴛ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss."
                )
                return
...
async def start_command(client, message):
    try:
        ...
        # Register user
        user_data = await register_user(message)  # ✅ pass message correctly

        if not user_data:
            await message.reply("❌ Failed to register. Please try again.")
            return

        # Handle referral if present and not a buy link
        if ref_code and not ref_code.startswith("buy_"):
            await handle_referral(client, message, ref_code)

        # Get custom keyboard/buttons
        reply_markup = await get_welcome_buttons(user_data["user_id"])

        # Send welcome message
        await message.reply(WELCOME_MSG, reply_markup=reply_markup)
...
#         # Get recent users
#         recent_users = await db.users.find().sort("join_date", -1).limit(10).to_list(length=None)
...
#         await message.reply("❌ ᴇʀʀᴏʀ ꜰᴇᴛᴄʜɪɴɢ ᴜꜱᴇʀ ꜱᴛᴀᴛꜱ!")
